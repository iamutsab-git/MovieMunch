...
const MovieCard = ({ movie, movies, title }) => {
  ...
  const { addToFavorites, loading } = useFavorites();
  ...
  const handleFavoriteClick = async (e, movieItem) => {
    e.stopPropagation();
    if (!loading) {  // Only allow if not already loading
        ...
        <div className="flex overflow-x-auto space-x-4 pb-4 px-4 scrollbar-hide">
          {movies.map((movieItem) => (
            <motion.div
              ...
              onClick={() => handleSinglePage(movieItem.id)}
            >
              <img
                src={
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={(e) => handleFavoriteClick(e, movieItem)}
